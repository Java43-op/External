...
// create class DiffieHellman to calculate the key for two persons  
class Program_2_DiffieHellman {
    // main() method start
    public static void main(String[] args) {
        ...
        System.out.println("Enter a Plain text(Number)");
        long plain = sc.nextInt();
        long cipher = encrypt(plain, ka);
        System.out.println("CT:" + cipher);
        System.out.println("PT:" + decrypt(cipher, kb));
    }

    // create calculatePower() method to find the value of x ^ y mod P
    static long encrypt(long plain, long ka) {
        return plain + ka;
    }

    static long decrypt(long cipher, long kb) {
        return cipher - kb;
    }
